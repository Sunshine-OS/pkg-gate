...
mod_existing(struct vfstab *vfsent, int fstab_entry, int is_remote)
{
	 ...
	 */
	if (is_remote && hasopt(vfsent->vfs_mntopts, "ro"))
		...
		if (access(VFS_TABLE, R_OK) == 0) {
			...
			while (!getvfsent(pp, vfs)) {
				...
				/*if (!(is_remote =
				    ...
				    0)
					continue;*/ /* Fix this for FreeBSD nullfs */
...
_getmntent(FILE *fp, struct mnttab *tp)
{
	...
	tp->mnt_special = "/dev/root"; //mp->fs_spec;
	tp->mnt_mountp = "/"; //mp->fs_file;
	tp->mnt_fstype = "ufs"; //mp->fs_vfstype;
	tp->mnt_mntopts = ""; //mp->fs_mntops;
	tp->mnt_time = "0";
	return 0;
}
