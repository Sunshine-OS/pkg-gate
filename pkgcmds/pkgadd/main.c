...
/*	from OpenSolaris "main.c	1.69	07/02/02 SMI"	*/

/*
 * Portions Copyright (c) 2007 Gunnar Ritter, Freiburg i. Br., Germany
 *
 * Sccsid @(#)main.c	1.10 (gritter) 2/26/07
 */
...
main(int argc, char **argv)
{
	...
	if (spoolDir != NULL) {
		...
		if (strcmp(spoolDir, VSADMDIR "/pkg") == 0) {
			progerr(ERR_SPOOLDIR_CANNOT_BE_SYS, VSADMDIR "/pkg");
			usage();
			return (1);
		}
	}
	...
	char **a_inheritedPkgDirs)
{
	...
	if (a_idsName != NULL) {
		arg[nargs++] = "-d";
		arg[nargs++] = a_idsName;
		dparts = ds_findpkg(a_idsName, pkginst);
		if (dparts < 1) {
			progerr(ERR_DSARCH, pkginst);
			quit(1);
			/* NOTREACHED */
		}
		arg[nargs++] = "-p";
	...
	/* if input data stream available, increment total parts read */

	if (a_idsName != NULL) {
		ds_totread += dparts; /* increment number of parts read */
	}
