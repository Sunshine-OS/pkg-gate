...
/*	from OpenSolaris "reqexec.c	1.25	06/02/27 SMI"	*/

/*
 * Portions Copyright (c) 2007 Gunnar Ritter, Freiburg i. Br., Germany
 *
 * Sccsid @(#)reqexec.c	1.3 (gritter) 2/25/07
 */
...
dup_respfile(void)
{
	...
	if (close(mkstemp(resppath = path)) < 0) {
		progerr(ERR_TMPRESP);
		return (99);
	}
	...
	boolean_t enable_root_user)
{
	 ...
	 * by the world
	 */
	if ((fd = open(resppath, O_WRONLY|O_CREAT|O_TRUNC, 0644)) < 0) { // REMOVED O_EXCL
		printf("Fd is equal to: %d \n Errno: %d \n", fd, errno);
...
chkexec(int update, char *script)
{
	...
	/* If there's no response file, create a fresh one. */
		 ...
		 * by the world
		 */
		fd = open(resppath, O_WRONLY|O_CREAT|O_TRUNC, 0644); // REMOVED O_EXCL
		if (fd < 0) {
			printf("Fd is equal to: %d \n Errno: %d \n", fd, errno);
			progerr(ERR_CRERESP, resppath);
			return (99);
